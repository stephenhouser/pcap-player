...
console.log(`Reading from: ${capture_file}`)


const udp_client = dgram.createSocket('udp4');

function select_packet(packet) {
	return port.includes(packet.dport);
}
		...
		pcap_session.on('packet', (raw_packet) => {
			...
			if (packet) {
				...
				try {
					...
					if (udp_packet && select_packet(udp_packet)) {
...
get_packets()
	.then(async (packets) => {
		for (i = 0; i < packets.length; i++) {
			...
			//console.log(p);
			console.log(p.sport, '-->', p.dport, ':', p.data);
			udp_client.send(p.data, p.dport, DEFAULT_HOST);
		}
	})
	.finally(() => {
		udp_client.close();
